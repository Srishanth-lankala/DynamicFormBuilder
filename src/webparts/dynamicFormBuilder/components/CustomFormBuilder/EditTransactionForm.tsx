...
const EditTransactionForm: React.FC<IDynamicFormBuilderProps> = ({ context }) => {
    ...
    const [ParentFormJSONCopy, setParentFormJSONCopy] = useState<any[]>([]);
    const [isButtonDisabled, setIsButtonDisabled] = useState(false);
    const [FormMasterJson, SetFormMasterJson] = useState<any[]>([])
    ...
    const [, setCurrentChildId] = useState<number>(0);
    ...
    const [childFileFolder, setChildFileFolder] = useState("")
    ...
    const [SelectedChildID, setSelectedChildSID] = useState<number>();

    const [deleteFileRun, setDeleteFileRun] = useState(true); // Track if file deletion has occurred
    ...
    useEffect(() => {
        ...
        const observeTableDOM = () => {
            const observer = new MutationObserver((mutations, obs) => {
                // Loop through each table definition
                tables.forEach((field: any) => {
                    ...
                    if (targetLabel && targetLabel.parentElement) {
                        ...
                        ReactDOM.render(
                            <TableComponent
                                ...
                                seqno={appSeqNo}
                                onRestrictionBreachUpdate={handleRestrictionBreachUpdate}
                            />,
                            parent
                        );
                    }
            ...
            // Observe the body for dynamic render updates
            observer.observe(document.body, {
                childList: true,
                subtree: true,
            });
    ...
    const initialfunforvisibility = async () => {
        const transactionItem = await sp.web.lists
            ...
            .select("*", "Title, Created, Author/Title,CategoryType,NotifyRequestor/EMail")
            .expand("Author", "NotifyRequestor")
            .get();

        //console.log("transactionItem in initialfunforvisibility", transactionItem);
        setTransactionItem(transactionItem);



        const formmasteritem = await sp.web.lists.getByTitle("FormMaster").items.filter(`AppCode eq '${transactionItem.AppCode}'`).top(1).get();
        const selectedFormJSON = JSON.parse(dataserviceobj.decryptjson(formmasteritem[0].FormJSON))
    ...
    const checkDomReady = () => {
        ...
        if (formContainer && trnxItem?.CurrentAppCode === "") {
    ...
    const fetchFormData = async () => {
        try {
            ...
            //console.log("finaldecrypttest", transactionItem);
            ...
            const IsParentForm = (transactionItem.CurrentAppCode) ? true : false;
            ...
            //console.log("ChildtransactionItems", ChildtransactionItems);
            ...
            // let  = [transactionItem, ...ChildtransactionItems];
            //console.log("TransactionItem______------", transactionItem)
            debugger;
            const parsedFormData = JSON.parse(transactionItem.FormData);
            let parentandchild = parsedFormData;
            // if (ChildtransactionItems) {
            //     let childFormdata = JSON.parse(ChildtransactionItems[0].FormData);
            //     parentandchild = [...parsedFormData, ...childFormdata];
            // }
            //console.log("parentandchildparentandchild", parentandchild);
            // const Paresedcompleteformdata = ;
            const updatedFormData = parentandchild.map((item: any) => {
            ...
            // const appSeqNo = IsParentForm ? ChildtransactionItems.filter(ele => ele.AppCode === transactionItem.CurrentAppCode)[0]?.SeqNo : transactionItem.SeqNo;
            // //console.log("appcode on which operated : ", appSeqNo);
            const appCodeValue = transactionItem.AppCode;
            // const currentAppCodeValue = transactionItem?.CurrentAppCode ?? appCodeValue;
            ...
            const formMasterItems = await sp.web.lists
                ...
                .get();
            //console.log("formMasterItemsformMasterItemsformMasterItems", formMasterItems)
            // setIsParentForm(formMasterItems[0].IsParentForm)
            formMasterItems[0].FormJSON = dataserviceobj.decryptjson(formMasterItems[0].FormJSON);
            ...
            if ((transactionItem.Status === "In-Progress" && Number(transactionItem.CurrentQueue) === highestLevel && userRole === "Creator") ||
                (transactionItem.Status === "In-Progress" && Number(transactionItem.CurrentQueue) === highestLevel && userRole === "Task Performer") ||
                (transactionItem.Status === "In-Progress" && Number(transactionItem.DestinationQueue) === highestLevel && userRole === "Task Performer")) {
                endflow = true;
            }
            // setLoading(true)
            const flowjson = await FlowData((thetransactionItem) ? thetransactionItem : transactionItem, levelsWithRoles, endflow)
            ...
            let RAWChildforms = [];
            if (IsParentForm) {
                RAWChildforms = await sp.web.lists.getByTitle("FormMaster").items.filter(`ParentAppCode eq '${appCodeValue}'`).get();
            ...
            //console.log("ChildformsChildformsChildforms", RAWChildforms)
            ...
            try {
                ...
                const CommentItems = await sp.web.lists
                    ...
                    .items.filter(`(AppCode eq '${CurrentChild ? CurrentChild.AppCode : appCodeValue}') and (SeqNo eq '${appSeqNo}')`)
    ...
    const updateChildTabData = async (childFormAppCode: string, childOrder: number | string, ChildFormJson: any) => {
        try {
            ...
            if (!childTransaction) {
                ...
                setChildFileFolder(childTransaction.SeqNo);
                setSelectedChildSID(childTransaction.ID)
    ...
    const deleteExistingChildFilesAndFolders = async () => {
        try {
            // Ensure fileFetchUrl is server-relative and normalized
            const normalizedFileFetchUrl = fileFetchUrl.startsWith('/')
                ? fileFetchUrl
            ...
            const sequenceFolderUrl = `${normalizedFileFetchUrl}/TableAttachments/${childFileFolder}`.replace(/\/+/g, '/');
            // Check if the main sequence folder exists
            let sequenceFolderExists = false;
            try {
                await sp.web.getFolderByServerRelativeUrl(sequenceFolderUrl).get();
    ...
    const handleSave = async (data: any) => {
        ...
        try {
            let [ParentAnswers, ChildAnswers] = AnswerJsonSeparator(FormMasterJson, updatedFormData)
                    ...
                    if (item.name && item.name.includes("file_upload")) {
                        ...
                        if (uplfiles) {
                            uplfiles.forEach((file: File) => {
                                if (file && file.name) {
                                    const isParent = FormMasterJson.find(elem => elem.id === item.id);
                                    if (isParent) {
            ...
            //console.log("ParentAnswers", ParentAnswers, "ChildAnswers", ChildAnswers);
            if (ParentAnswers[0]) {
                ...
                console.log("ChildFormDetails", childselected, Childids);
                //console.log('childselected', childselected);
                //console.log("childtrascationItems in save : ", ChildtransactionItems);
            }
            if (IsParentForm) {
                debugger;
                const formMasterItem2 = await sp.web.lists.getByTitle("FormMaster")
                    .items.select("FormPrefix,isWorkflowRequired,AppCode,AppName")
                    .filter(`AppCode eq '${record.CurrentAppCode}'`).get();
                const newTitle = (ChildtransactionItems.length < 2) ? `${formMasterItem2[0].FormPrefix}/Draft` : record.Title;
                // const newTitle = `${formMasterItem2[0].FormPrefix}/Draft`'
                const childid = ChildtransactionItems.filter(ele => ele.AppCode === record.CurrentAppCode)[0]
                ...
                if (!childid) {
                    ...
                    await sp.web.lists.getByTitle("WorkFlowProcessData").items.add({
                        ...
                        AppName: formMasterItem2[0].AppName,
                        AppCode: formMasterItem2[0].AppCode,
                    ...
                    setChildids(childid.Id);
                    await sp.web.lists.getByTitle("WorkFlowProcessData").items.getById(Number(childid.Id)).update({
                        FormData: JSON.stringify(ChildAnswers),
                        AppName: formMasterItem2[0].AppName,
                        AppCode: formMasterItem2[0].AppCode,
                        ParentAppCode: appCode,
                ...
                //     await sp.web.lists
                //         .getByTitle("WorkFlowProcessData")
                //         .items.getById(Number(childid.Id))
                //         .attachmentFiles.add(file.name, file.file);
                // }


                const ChildFolderName = `NoteAttach/${childseqNo}`;
                try {
                    // Check if folder exists
                    await sp.web.getFolderByServerRelativeUrl(ChildFolderName).get();
                } catch (error) {
            ...
            if (IsParentForm) {
                const ParentFolderName = `NoteAttach/${fileFolder}`;
                try {
                    await sp.web.getFolderByServerRelativeUrl(ParentFolderName).get();
                } catch (error) {
                ...
                const folderName = `NoteAttach/${fileFolder}`;
                try {
    ...
    const handleUpdateForm = async (data: any) => {
        const { recordscount, trnxRecLimitReached } = await fetchWorkflowLimit(domainfield, appCode);
        if (trnxRecLimitReached) {
            toast.current?.clear();
            toast.current?.show({
                severity: 'warn',
                summary: 'Freemium subscription limit reached',
                detail: `${appName}: You reached ${recordscount} transactions. Please upgrade your license.`,
                life: 8000,
                style: { height: 100 }
            });
            return;
        ...
        try {
            ...
            //console.log("inupdateform formdata and result1", formData, result1);
            ...
            result1.forEach((item: any) => {
                ...
                // console.log("childselected", childselected);
                // const childformmaster = (ChildFormDetails.length>0 )? ChildFormDetails.filter((data:any) => data.AppCode === childselected) : [];
                // const childformmasterjson = JSON.parse(childformmaster[0].FormJSON );
                // console.log("childformmasterjson",childformmaster);
                // console.log("childdetails",ChildFormDetails);
                // console.log("childformmasterjson",childformmasterjson);
                // const FormMasterJsonwithchild = [...FormMasterJson, ...childformmasterjson];
                // console.log("combinedformmasterjson",FormMasterJsonwithchild);  


                // const Formstructure = IsParentForm ? FormMasterJsonwithchild : FormMasterJson;
                if (Array.isArray(formJSON) && Array.isArray(mappedfile)) {
                    debugger
                    //console.log("insidefileupload", FormMasterJson)
                    let formjsonitem = formJSON.find((formjson: any) => {
            ...
            const routingRulesItems = await getRoutingRules(IsParentForm ? childselected : appCode, buttonAction);
            ...
            const mappingMasterItem = await sp.web.lists.getByTitle("MappingMaster").items
                ...
                .expand("Users_x002f_Groups")
                .filter(`(Level eq '${DestinationQueue}') and (AppCode eq '${IsParentForm ? childselected : appCode}')`)
                .get();
                ...
                data.forEach((item: any) => {
                    if (item.name && item.name.includes("file_upload")) {
                        ...
                        if (uplfiles) {
                            uplfiles.forEach((file: File) => {
                                if (file && file.name) {
                                    const isParent = ParentFormJSONCopy.find(elem => elem.id === item.id);
                                    if (isParent) {
                                        parentFiles.push({ name: file.name + "name" + item.name, file });
                                    } else {
                                        childFiles.push({ name: file.name + "name" + item.name, file });
            ...
            if ((returnedStatus === "Returned") && (ZeroDestinationQueue === "0")) {
                ...
                if (commentValidationValue === "true") {
                    ...
                    try {
                        await sp.web.lists.getByTitle("WorkFlowProcessData").items.getById(Number(id)).update({
                            ...
                            // FormData: JSON.stringify(finalencjson),
                            FormData: JSON.stringify(ParentAnswers),
                        ...
                        debugger;
                        if (IsParentForm) {
                            const childid = ChildtransactionItems.filter((item: any) => item.AppCode === record.CurrentAppCode)[0]
                            //console.log("Child ids :", childid);
                            setChildids(childid);
                            await sp.web.lists.getByTitle("WorkFlowProcessData").items.getById(Number(childid.Id)).update({
                                FormData: JSON.stringify(ChildAnswers),
                    ...
                    if (IsParentForm) {
                        await deleteExistingChildFilesAndFolders();
                    }
                    tables.forEach(async (table: any) => {
                        const attachcolumnName = table.columns.find((column: any) => column.columntype === "Attachment")?.headername
                        const boolvalue: boolean = table.rows.find((row: any) => row[attachcolumnName].name.length > 0)
                    ...
                    if (!IsParentForm) {
                        //this is form single form noteattach attachment                  
                        const folderName = `NoteAttach/${fileFolder}`;
                        try {
                            await sp.web.getFolderByServerRelativeUrl(folderName).get();
                        } catch (error) {
                        ...
                        //this is for Parent form noteattach document
                        const ParentFolderName = `NoteAttach/${fileFolder}`;
                        try {
                            await sp.web.getFolderByServerRelativeUrl(ParentFolderName).get();
                        } catch (error) {
                        ...
                        // this is for child form noteattach document
                        const ChildFolderName = `NoteAttach/${appSeqNo}`;
                        try {
                            await sp.web.getFolderByServerRelativeUrl(ChildFolderName).get();
                        } catch (error) {
                            await sp.web.folders.add(ChildFolderName);
                ...
                const newTitle = (ChildtransactionItems.length < 2) ? `${formMasterItem2[0].FormPrefix}/${monthYear}/${latestNumber + 1}` : record.Title;
                debugger;
                ...
                try {
                    ...
                    if (IsParentForm) {
                        ...
                        if (childid) {
                            setChildids(childid);
                            //console.log("New Title for child form: ", newTitle);
                            await sp.web.lists.getByTitle("WorkFlowProcessData").items.getById(Number(childid.Id)).update({
                                Title: newTitle,
                                FormData: JSON.stringify(ChildAnswers),
                                CurrentQueue: CurrentQueue,
                ...
                if (IsParentForm) {
                    ...
                    //this is for single form noteattach document
                    const FolderName = `NoteAttach/${fileFolder}`;
                    try {
                        await sp.web.getFolderByServerRelativeUrl(FolderName).get();
                    } catch (error) {
    ...
    const handleDocumentDelete = async (fileName: string, fileId: string) => {
        ...
        let cleanFileName = fileName.replace(/^\d+\.\s*/, "");
        ...
        try {
            ...
            if (!IsParentForm) {
                ...
                await fetchFormData();
                //   setDeleteFileRun(true);
                // setFormData(formdattttt)
                // setMappedfile(parsedFormDataArrayyyy);

                setDeleteFileRun(false);

                // Reset deleteFileRun to false after rendering 
                setTimeout(() => {
                    setDeleteFileRun(true);
                }, 1000);
            }
    ...
    const handleChildDocumentDelete = async (fileName: string, fileId: string) => {
        // debugger
        //console.log("fileId--------------- nin delete -------- ", fileId, mappedfile)
        ...
        // Ensure correct relative path (must start with `/sites/...`)
        let fileRelativePath = `/${fileFetchUrl}/NoteAttach/${childFileFolder}/${encodedFileName}`;
        ...
        try {
            ...
            // Fetch the list item from SharePoint
            const childtransactionItem = await sp.web.lists
                .getByTitle("WorkFlowProcessData")
                .items.getById(Number(SelectedChildID))
                .select("*", "Title, Created, Author/Title", "FormData")
                .expand("Author")
                .get();
            ...
            // Parse and update FormData
            const parsedFormData = JSON.parse(childtransactionItem.FormData);
            const updatedFormData = parsedFormData.map((item: any) => {
                if (item.name.startsWith("file_upload")) {
                    return {
                        ...item,
            ...
            // Update the list item with modified FormData
            await sp.web.lists.getByTitle("WorkFlowProcessData").items.getById(Number(SelectedChildID)).update({
                FormData: JSON.stringify(updatedFormData),
            });

            // Remove the file from UI state
            setChildAttachmentFiles(prevFiles => prevFiles.filter(file => file.name !== fileName));
    ...
    const handlereqCancel = async () => {
        ...
        if (comment.trim().length < 2) { console.log("invalid Comment"); toast.current?.show({ severity: 'error', detail: messages, life: 1000 }); }
            ...
            try {
                const newreqcancel = await sp.web.lists
                    .getByTitle("WorkFlowProcessData")
                ...
                if (IsParentForm) {
                    ...
                    console.log("Child ids for cancel (-_x) :", childid, newreqcancel);
                    await sp.web.lists
                        .getByTitle("WorkFlowProcessData")
                        .items.getById(Number(childid)).update(
                            { Status: 'Cancelled', CurApproverId: null, DestinationQueue: '' });
                }
    ...
    const handleDeleteRequest = async () => {
        try {
            ...
            debugger;
            const newreqcancel = await sp.web.lists.getByTitle("WorkFlowProcessData").items.getById(Number(id)).delete();
            if (IsParentForm) {
                const childid = ChildtransactionItems[0].Id;
                console.log("Child ids for Delete (x_x) :", childid, newreqcancel, ChildtransactionItems);
                await sp.web.lists.getByTitle("WorkFlowProcessData").items.getById(Number(childid)).delete();
            }
            toast.current?.show({
                severity: 'success',
                summary: '',
    ...
    const childvisibilityHandler = (completeformjson: any, completeformanswers: any) => {
        const dependentFields = completeformjson.filter((ele: any) => { //All child fields having conditions
            return ele?.dependentVisibility
        })
        SetChildFieldJsons(dependentFields)
        // const dependentShowFields = completeformjson.filter((ele: any) => { //Child fields which should be hidden initially
        //     return ele?.dependentVisibility && ele?.controlVisibility
        // })
        //console.log("dependentFields", dependentFields)
        // console.log("dependentShowFields", dependentShowFields)
        ...
        const SourceFields = completeformjson.filter((ele: any) => {
            return ele?.label && SourceFieldNames.includes(stripHtmlTags(ele?.label))
        })
        //console.log("SourceFieldsmulti", SourceFields)
        ...
        //console.log("transformedarraymulti", transformedArray)
        SetSrcFieldAnswer(transformedArray)
        funforVisibility(completeformanswers);
    }
                ...
                <div style={{ display: 'flex', flexDirection: 'column' }}>
                    ...
                    <div style={{ display: 'flex' }}>
                        ...
                        <div className='FC_mainDiv' style={{ width: '100%' }}>
                            ...
                            {(IsParentForm && deleteFileRun) && <ParentWithChildren
                                parentName="Form Panel"
                                children={filteredChildFormDetails.map((ele) => ({ id: ele.Id, name: ele.AppName, ChildFormJson: JSON.parse(ele.FormJSON), AppCode: ele.AppCode, ChildOrder: ele.ChildOrder }))} // Assuming ChildFormDetails has AppCode and AppName
                                onChildClick={async (child: any) => {
                                    ...
                                    //console.log("child selected", child.AppCode, child.id);



                                    let completeformjson = [...ParentFormJSONCopy, ...ChildFormJson];
                                    //console.log("ParentFormjsoncopy", ParentFormJSONCopy, child.ChildFormJson);
                                    // const parentformJsons = ParentFormJSONCopy;
                                    ...
                                    //console.log("MultiFormSeqNooooooooooooooooooooooooooooooooooooooooooooooooooooooooo", currentchildtransactionitem?.SeqNo)
                                    setTables(currentchildtransactionitem?.TableJSON ? JSON.parse(currentchildtransactionitem?.TableJSON) : []);
                                    setTimesheettables(currentchildtransactionitem?.TimeSheetJSON ? JSON.parse(currentchildtransactionitem?.TimeSheetJSON) : []);

                                    // const isAccordionVisible = !currentchildtransactionitem || currentchildtransactionitem.Status?.toLowerCase() === "draft";
                                    // //console.log("currentchild---->", Boolean(!currentchildtransactionitem));
                                    //console.log("setIsAccordionVisible---->", isAccordionVisible);
                                    // setIsAccordionVisible(isAccordionVisible);                    
                                    // const ChildFormJsons = child.ChildFormJson;
                                    // setchildformJsons(ChildFormJsons);
                                    ...
                                    // const parentformJsons, childformJsons = 
                                    ...
                                    if (currentchildtransactionitem.Status === "In-Progress" && child.AppCode.slice(-3) === "-1C") {
                                        setCancelrequest(true)
                                    }
                                    else { setCancelrequest(false) }
                                    childvisibilityHandler(completeformjson, completeformanswers);
                                    // void fetchFormData();
                                ...
                                <div className='fo-mid1'>
                                    {selectTab === 'AllDetails' && (
                                        <>
                                            <div className='p-tabview-panel' style={{ paddingTop: 6 }}>
                                                ...
                                                {/* {isAccordionVisible || buttonView || ResubmitButton ?

                                            
                                                <div className="form-component-mode">

                                                    <ReactFormGenerator
                                                        // key={JSON.stringify(isAccordionVisible)}
                                                        onBlur={(e) => handleBlur(e)}
                                                ...
                                                <div style={{ marginLeft: '1.5rem' }}>
                                                    ...
                                                    <div className="comment-box" >
                                                        ...
                                                        <div style={{ flexShrink: 0, flexBasis: "20%" }}>
                                                            ...
                                                            <button onClick={handleCommnetSaveOnly} style={{ backgroundColor: "#0E8EBE", borderRadius: "4px", fontSize: "12px", color: "white", padding: "6px 17px", margin: "14px 20px", alignItems: "center" }}>Save</button>
                                                        </div>
                                                ...
                                                attachmentFiles.map((file, index) => (
                                                    <div key={index}>
                                                        ...
                                                        <section style={{ display: "flex", justifyContent: "space-between" }}>
                                                            ...
                                                            {buttonView ? <button className="doc_delete" onClick={() => handleDocumentDelete(file.name, file.id)}>Delete</button> : ""}
                                    ...
                                    {(selectTab === 'Attachment' && IsParentForm) && (
                                        <div className='p-tabview-panel'>
                                            ...
                                            <div>
                                                ...
                                                {attachmentFiles.length === 0 ? (
                                                    <li style={{ marginTop: '10px', marginLeft: '10px' }}>No attachments available</li>
                                                ) : (
                                                    attachmentFiles.map((file, index) => (
                                                        <div key={index}>
                                                            <section style={{ display: "flex", justifyContent: "space-between" }}>
                                                                <p className='Attachment_fileName' style={{ background: "#212b4608", borderTop: "2px solid #212b46;" }}>{file.name.split("name")[0]}</p>
                                                                {buttonView ? <button className="doc_delete" onClick={() => handleDocumentDelete(file.name, file.id)}>Delete</button> : ""}
                                                            </section>
                                                            {file.name.endsWith('.pdf') ? (
                                                                <embed src={file.url} width="100%" height="600px" type="application/pdf" />
                                                ...
                                                {childAttachmentFiles.length === 0 ? (
                                                    ...
                                                    childAttachmentFiles.map((file, index) => (
                                                        <div key={index}>
                                                            <section style={{ display: "flex", justifyContent: "space-between" }}>
                                                                <p className='Attachment_fileName' style={{ background: "#212b4608", borderTop: "2px solid #212b46;" }}>{file.name.split("name")[0]}</p>
                                                                {buttonView ? <button className="doc_delete" onClick={() => handleChildDocumentDelete(file.name, file.id)}>Delete</button> : ""}
                                                            </section>
                                                            {file.name.endsWith('.pdf') ? (
                                                                <embed src={file.url} width="100%" height="600px" type="application/pdf" />
                                                            ) : file.name.endsWith('.jpg') || file.name.endsWith('.jpeg') || file.name.endsWith('.png') ? (
                                    ...
                                    {ResubmitButton ?
                                        <div className='form-btn'>
                                            ...
                                            <button disabled={isButtonDisabled} className='Sbmt-btn all-btn' onClick={async () => { if (formData) { await handleUpdateForm(formData) } }}>Re-Submit</button>
                                        ...
                                        buttonView ?
                                            <div className='form-btn'>
                                                ...
                                                <button disabled={isButtonDisabled} className='Sbmt-btn all-btn' onClick={async () => { if (formData) { await handleUpdateForm(formData) } }}>Submit</button>
                                                <button className='all-btn' onClick={(e) => { handlecancelchange("delete") }}>Delete Request</button>
                                                <button className="all-btn" onClick={handleCancel}>Close</button>

                                            </div> : <div className='form-btn'>
                                                {(reopenbuttonview && (recentStatus === "Completed" || recentStatus === "Rejected")) && <button className='Sbmt-btn all-btn' onClick={() => setisreopen(true)}>Re open</button>}
                                                {Cancelrequest && (<button className='all-btn' onClick={(e) => { handlecancelchange("cancel") }}>Cancel Request</button>)}
                                                <button className="all-btn" onClick={handleCancel}>Close</button>
                            ...
                            {recentStatus !== "Draft" ? <ChatBox context={context} appSeqNo={IsParentForm ? multiFormSeqNo : appSeqNo} userName={userName} onChatResponse={handleChatResponse} appcode={AppCodevalue} isSameDomain={isSameDomain} /> : ""}
                        </div>
                    </div>
