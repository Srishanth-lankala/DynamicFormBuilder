 ...
 const ChildForm: React.FC<IDynamicFormBuilderProps> = ({ context }) => {
    ...
    const handleTableUpdate = (updatedTable: TableData) => {
        setTables(prevTables => {
            console.log('Previous tables:', prevTables);
...
debugger;
    ...
    const rendertimesheetedit = () =>{
        debugger;
        const TimeTableStructureJsons = ChildFormJsons.filter((ele:any )=> ele.element === "Download" && ele?.text === "Timesheet")
            const TransformedTimeTableJsons = TimeTableStructureJsons?.map((ele : any) => {
              return ({
                id: ele.id,
            ...
            TransformedTimeTableJsons.forEach((field: any) => {
              ...
              if (targetLabel && targetLabel.parentElement) {
                ...
                targetLabel.remove()
                ReactDOM.render(
                  <TimesheetComponent tableConfig={field} onTableUpdate={handletimesheetTableUpdate} onRestrictionBreachUpdate={handleTimeRestrictionBreachUpdate}/>,parent
    ...
    const rendertableeditmode = () => {
        debugger;
        const TableStructureJsons = ChildFormJsons.filter((ele: any) => ele.element === "Label" && ele?.text === "Table")
        const TransformedTableJsons = TableStructureJsons?.map((ele: any) => {
            return ({
                id: ele.id,
                content: stripHtmlTags(ele.content),
                text: ele.text,
        ...
        console.log("Tables setting to state", TransformedTableJsons)
    ...
    },[isAccordionVisible,ChildFormJsons])
    ...
    useEffect(() => {
        if (filteredChildFormDetails.length > 0 && ChildtransactionItems.length > 0 &&
            ...
            //console.log("First visible child form:===============================", firstChild);
    ...
    const fetchFormData = async () => {
        try {
            ...
            const ChildtransactionItems = await sp.web.lists.getByTitle("WorkFlowProcessData").items
                ...
                .select("*", "Title", " Created", "Domain", "CurrentAppCode", "AuthorId", "Author/Title", "CategoryType", "NotifyRequestor/Id","NotifyRequestor/EMail", "TableJSON", "TimeSheetJSON", "CurApprover/Title")
                .expand("Author", "NotifyRequestor", "CurApprover")
                .orderBy("Id", true)
                .get()
            ...
            const RAWchildformdetials = await sp.web.lists.getByTitle("FormMaster").items
                .filter(`ParentAppCode eq '${transactionItem.AppCode}'`)
                .select("*", "Title", "IsParentForm", "ChildOrder", "NotifyTo/Title", "NotifyToId")
                .expand( "NotifyTo")
                .orderBy("ChildOrder", true)
                .get();
                 const childformdetials = RAWchildformdetials?.map((item:any)=>{
                return { ...item, FormJSON: dataserviceobj.decryptjson(item.FormJSON)}
            })
            SetChildFormDetails(childformdetials);
            // setLoading(false)
    ...
    const handletabledata = (Formstructure : any ,Tabledatastate : any,Timesheetdatastate : any ,isAccordionVisible: boolean) => {
        debugger;
        const formjsondatacurrent: any =(isAccordionVisible)? Formstructure : ChildFormJsons;
        if(isAccordionVisible || Tabledatastate === "[]"){

        const TableStructureJsons = formjsondatacurrent?.filter((ele: any) => ele.element === "Label" && ele?.text === "Table")
        const TransformedTableJsons = TableStructureJsons?.map((ele: any) => {
            return ({
                id: ele.id,
                content: stripHtmlTags(ele.content),
        ...
        setTables(TransformedTableJsons || []);}
            ...
            setTables(JSON.parse(Tabledatastate) || []);}
        if(isAccordionVisible || Timesheetdatastate === "[]"){
        const TimeTableStructureJsons = formjsondatacurrent?.filter((ele:any )=> ele.element === "Download" && ele?.text === "Timesheet")
            const TransformedTimeTableJsons = TimeTableStructureJsons?.map((ele : any) => {
              return ({
                id: ele.id,
                content: stripHtmlTags(ele.content),
            ...
            setTimesheettables(TransformedTimeTableJsons || []);}
                ...
                setTimesheettables(JSON.parse(Timesheetdatastate) || []);}
            }
    // }, [isAccordionVisible,ChildFormJsons])
    // useEffect(() => {
        ...
        try {
            ...
            if ((returnedStatus === "Returned") && (ZeroDestinationQueue === "0")) {
                ...
                if (commentValidationValue === "true") {
                    ...
                    const finalUpdatedFormData = assignMissingFiles(updatedFormData, childAttachmentFiles);
    ...
    //         await sp.web.lists.getByTitle("WorkFlowProcessData").items.getById(Number(childtransactionItem)).update({
    //             FormData: JSON.stringify(updatedFormData),
    //         });
    ...
    try {
        // Ensure that SelectedChildID is valid
        if (!SelectedChildID || isNaN(SelectedChildID)) {
            console.error("Invalid SelectedChildID, cannot proceed with delete.");
            toast.current?.show({ severity: 'error', summary: '', detail: `Error deleting file`, life: 2000 });
            return;
        }
    ...
    const updateChildTabData = async (childFormAppCode: string, childOrder: number | string, ChildFormJson: any, matchedChildData?: any) => {
        try {
            ...
            if (!childTransaction) {
                ...
                setSelectedChildID(childTransaction?.ID)
    ...
    {(loading) ? <LoadingSpinner></LoadingSpinner> : (record.Domain === domainfield && (userId === trnxItem?.CurApproverId || isAdmin) ?                
                            ...
                            {IsParentForm && (
                                <ParentWithChildren
                                    ...
                                    onChildClick={async (child) => {
                                        ...
                                        const matchedChild = ChildtransactionItems.find(ele => ele.AppCode === child.AppCode);                                      
                                        ...
                                        const childOrder : any = child.ChildOrder || 1;
                                        const currentchildanswerdata = matchedChild? JSON.parse(matchedChild.FormData): [];
                                        ...
                                        //console.log("Matched Child Table JSON-------------:", matchedChild);
                                        console.log("Matched Child Table setting-------------:",JSON.parse(matchedChild?.TableJSON || "[]"));
                                        handletabledata(child.ChildFormJson,matchedChild?.TableJSON,matchedChild?.TimeSheetJSON,!matchedChild)
                                        setRecentStatus(matchedChild?.Status);
                                        //console.log("Form Data Updated ✅", {completeformanswers,completeformjson});
                                    }}
                                />
                            )}
                                    ...
                                    {selectTab === 'AllDetails' && (
                                        <>
                                            <div className='p-tabview-panel' style={{ paddingTop: 6 }}>
                                                ...
                                                {/* ChildtransactionItems?.find(ele => ele.AppCode === childappselected) */}
                                                ...
                                                <div style={{ marginLeft: '1.5rem' }}>
                                                    ...
                                                    <div className="comment-box">
                                                        ...
                                                        <div style={{ flexShrink: 0, flexBasis: "20%" }}>
                                                            ...
                                                            <button onClick={handleCommnetSaveOnly} 
                                                            // style={{ backgroundColor: "green", color: "white", padding: "6px 17px", fontWeight: "bold", margin: "14px 20px", alignItems: "center" }}
                                                            className='all-btn'
                                                            >Save</button>
                                                        </div>
                                                    ...
                                                    childAttachmentFiles.map((file, index) => (
                                                        <div key={index}>
                                                            <section style={{ display: "flex", justifyContent: "space-between" }}>
                                                                ...
                                                                {isAccordionVisible||isChildReturned||buttonView ? <button className="doc_delete" onClick={() => handleChildDocumentDelete(file.name, file.id)}>Delete</button> : ""}
                                                            </section>
                                                            {file.name.endsWith('.pdf') ? (
                                                                <embed src={file.url} width="100%" height="600px" type="application/pdf" />
                                                            ) : file.name.endsWith('.jpg') || file.name.endsWith('.jpeg') || file.name.endsWith('.png') ? (
